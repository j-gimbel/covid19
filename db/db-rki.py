...
class DB:
    ...
    def update(self):
        ...
        # bundesland
        csv_entries_bundesland = (
            self.session.query(models.Inserted_csv_File)
            .filter_by(data_type="Bundesland")
            .all()
        )
        csv_files_in_db = []
        ...
        for f in csv_files_on_disk:
            ...
            self.insert_bundeslaender_data_from_csv(
                date=date, filepath="downloads/" + f
            )

        # Landkreise
        csv_entries_landkreis = (
            self.session.query(models.Inserted_csv_File)
            .filter_by(data_type="Landkreis")
            .all()
        )
        csv_files_in_db = []
        ...
        for f in csv_files_on_disk:
            ...
            self.insert_landkreise_data_from_csv(date=date, filepath="downloads/" + f)

        # Faelle
        csv_entries_fall = (
            self.session.query(models.Inserted_csv_File)
            .filter_by(data_type="Fall")
            .all()
        )
        csv_files_in_db = []
        ...
        """
        - alle Inserted_csv_File von DB abfragen
        - alle files in / downloads globben
        - delta finden 
        - aufrufen
